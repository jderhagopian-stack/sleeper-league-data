...
Pipeline 1.14 retains adaptive deep confirmation and adds roster-aware trade
resolution. Hypothetical trades are legalized before simulation so any forced
active-roster cuts are included in lineup outcomes, strategic value, buyer
acceptance analysis, option comparisons, and the final PDF.
"""
...
MODEL_VERSION='FSFFL-Trade-Query-Pipeline-1.16'
EXPECTED_ANALYSIS_MODEL='FSFFL-Counter-Market-Sweep-1.24'
REPORT_VERSION='FSFFL-Trade-Decision-Report-1.9'
...
def sensitivity_reasons(report):
    ...
    if deltas and abs(deltas[0]-750)<=600:reasons.append('best_option_near_accept_vs_shop_boundary')
    scores=sorted([sf(x.get('post_sim_score')) for x in rows],reverse=True)
    if len(scores)>=2 and abs(scores[0]-scores[1])<=600:reasons.append('top_recommendations_near_tie')
