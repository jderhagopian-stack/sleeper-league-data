...
PDF_RENDERER=Path('script/render_trade_decision_report_v13.py')
MODEL_VERSION='FSFFL-Trade-Query-Pipeline-1.10'
EXPECTED_ANALYSIS_MODEL='FSFFL-Counter-Market-Sweep-1.21'
REPORT_VERSION='FSFFL-Trade-Decision-Report-1.3'
...
def summary(report):
 ...
 short=f"{label}. Current-offer impact: {float(d.get('expected_wins') or 0):+.2f} expected wins, {float(d.get('championship_probability') or 0)*100:+.1f} pts championship probability, {float(st.get('strategic_value_delta') or 0):+,.0f} state-aware strategic value."
...
def main():
 ...
 payload={'pipeline_model_version':MODEL_VERSION,'analysis_model_version':r.get('model_version'),'report_model_version':REPORT_VERSION,'recommended_next_action':r.get('recommended_next_action'),'suggested_counteroffer_count':len(r.get('suggested_counteroffers') or []),'market_sweep_alternative_count':len(r.get('market_sweep_alternatives') or []),'short_answer':summary(r),'json_report':str(jp),'pdf_report':str(pp),'canonical_model_entry_point':str(MARKET_SWEEP),'delivery_policy':'Always return the short answer and attach/share the natural-language explanatory PDF report for a trade query.'}
