#!/usr/bin/env python3
"""Run FSFFL trade analysis and emit JSON + natural-language PDF + short answer.
...
realistic multi-asset search. Presentation uses Trade Decision Report 1.2,
which turns those model outputs into a concise GM-style narrative while
...
PDF_RENDERER=Path('script/render_trade_decision_report_v12.py')
MODEL_VERSION='FSFFL-Trade-Query-Pipeline-1.9'
...
def summary(report):
    ...
    if same and set(same.get('return_assets') or [])!=set(current.get('return_assets') or []):short+=f" Same-partner counter candidate: receive {', '.join(same.get('return_asset_names') or [])}."
    short+=f" Market sweep produced {len(top)} alternative{'s' if len(top)!=1 else ''}." if top else ' No league-wide alternative cleared the current sweep filters.'
    return short
def main():
    ...
    payload={'pipeline_model_version':MODEL_VERSION,'analysis_model_version':r.get('model_version'),'report_model_version':'FSFFL-Trade-Decision-Report-1.2','recommended_next_action':r.get('recommended_next_action'),'short_answer':summary(r),'json_report':str(jp),'pdf_report':str(pp),'canonical_model_entry_point':str(MARKET_SWEEP),'delivery_policy':'Always return the short answer and attach/share the natural-language explanatory PDF report for a trade query.'}
