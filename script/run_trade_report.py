#!/usr/bin/env python3
"""Run FSFFL trade analysis and emit JSON + explanatory PDF + short answer.
...
realistic multi-asset search. Presentation uses Trade Decision Report 1.1,
which explains the decisive strategic drivers, separates same-partner counters
from league-wide alternatives, and reports the actual number of alternatives.
"""
from __future__ import annotations
import argparse,json,subprocess,sys
from pathlib import Path
MARKET_SWEEP=Path('script/run_trade_market_sweep_v26.py')
PDF_RENDERER=Path('script/render_trade_decision_report_v11.py')
MODEL_VERSION='FSFFL-Trade-Query-Pipeline-1.8'
EXPECTED_ANALYSIS_MODEL='FSFFL-Counter-Market-Sweep-1.20'
def run(cmd):subprocess.run(cmd,check=True)
def summary(report):
    action=str(report.get('recommended_next_action') or 'REVIEW');current=report.get('current_offer_evaluation') or {};sim=current.get('simulation') or {};delta=sim.get('focus_delta') or {};strategic=sim.get('strategic') or {};top=report.get('top_5_alternatives') or [];same=report.get('best_same_partner') or {}
    ...
    short=f"{label}. Current-offer impact: {float(delta.get('expected_wins') or 0):+.2f} expected wins, {float(delta.get('championship_probability') or 0)*100:+.1f} pts championship probability, {float(strategic.get('strategic_value_delta') or 0):+,.0f} state-aware strategic value."
    if same and set(same.get('return_assets') or [])!=set(current.get('return_assets') or []):
        short+=f" Same-partner counter candidate: receive {', '.join(same.get('return_asset_names') or [])}."
    if top:
        short+=f" Market sweep produced {len(top)} alternative{'s' if len(top)!=1 else ''}."
    else:
        short+=' No league-wide alternative cleared the current sweep filters.'
    return short
def main():
    ...
    payload={'pipeline_model_version':MODEL_VERSION,'analysis_model_version':r.get('model_version'),'report_model_version':'FSFFL-Trade-Decision-Report-1.1','recommended_next_action':r.get('recommended_next_action'),'short_answer':summary(r),'json_report':str(jp),'pdf_report':str(pp),'canonical_model_entry_point':str(MARKET_SWEEP),'delivery_policy':'Always return the short answer and attach/share the explanatory PDF report for a trade query.'}
