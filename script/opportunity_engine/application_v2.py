...
def build_board(source,a,reviews):
    b=v1.build_board(source,focus_user_id=a.focus_user_id,portfolio_depth=0,seed=a.seed,trade_reviews=reviews); frontier=negotiation_frontier.build(source.get('top_cross_channel_options') or []); b['model_version']=MODEL_VERSION; b['negotiation_frontier']=frontier
    ...
    b.setdefault('policy',{}).update({'negotiation_frontier_creates_new_utility':False,'heuristic_acceptance_fit_is_probability':False,'theoretical_counterparty_failures_can_be_headline_action':False,'portfolio_search_excludes_theoretical_counterparty_failures':True}); b.setdefault('capability_status',{}).update({'negotiation_frontier':True,'actionable_vs_explore_vs_theoretical_trade_separation':True}); b.setdefault('provenance',{})['negotiation_frontier_bilateral_authority']='canonical governed counterparty shared utility'; return b
def main():
