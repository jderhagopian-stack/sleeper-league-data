...
MODEL_VERSION='FSFFL-Opportunity-Engine-2.2'
...
def build_board(source,a,reviews):
    ...
    b['model_version']=MODEL_VERSION; b['strategic_posture']=copy.deepcopy(source.get('strategic_posture') or {}); b['negotiation_frontier']=frontier; b['best_price_overlap']=frontier.get('best_price_overlap')
    overlap_keys={(str(x.get('seller_user_id') or ''),str((x.get('target') or {}).get('asset_id') or '')) for x in (frontier.get('target_price_frontiers') or []) if x.get('price_overlap_exists')}
    actionable_trades=[
        x for x in (frontier.get('actionable_negotiations') or [])+(frontier.get('negotiation_targets') or [])
        if (str(x.get('seller_user_id') or ''),str((x.get('target') or {}).get('asset_id') or '')) in overlap_keys
    ]
    ...
    actionable_trade_signatures={
        (str(x.get('seller_user_id') or ''),str((x.get('target') or {}).get('asset_id') or ''),tuple(sorted(str(a.get('asset_id') or '') for a in (x.get('outgoing') or []))))
        for x in actionable_trades
    }
    filtered_views={}
    for key,row in (b.get('specialized_views') or {}).items():
        if not row:
            filtered_views[key]=None
            continue
        if row.get('channel')=='TRADE':
            sig=(str(row.get('seller_user_id') or ''),str((row.get('target') or {}).get('asset_id') or ''),tuple(sorted(str(a.get('asset_id') or '') for a in (row.get('outgoing') or []))))
            filtered_views[key]=row if sig in actionable_trade_signatures else None
        elif row.get('channel')=='WAIVER':
            filtered_views[key]=row if float(row.get('team_improvement_score') or 0)>0 else None
        else:
            filtered_views[key]=row
    b['specialized_views']=filtered_views
    ...
    b['portfolio_optimization']=build_portfolio(source,a.focus_user_id,a.portfolio_depth,a.portfolio_max_moves,a.portfolio_beam_width,a.portfolio_sims,a.portfolio_confirm_sims,a.portfolio_confirm_top,a.seed,a.strategic_posture); b['prospective_validation']=_prospective(source)
    ...
    return b
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--focus-user-id',required=True); ap.add_argument('--output',required=True); ap.add_argument('--team-improvement-input'); ap.add_argument('--quick-sims',type=int,default=500); ap.add_argument('--confirm-sims',type=int,default=2500); ap.add_argument('--trade-screen',type=int,default=75); ap.add_argument('--waiver-screen',type=int,default=50); ap.add_argument('--confirm-top',type=int,default=15); ap.add_argument('--trade-packages-per-target',type=int,default=12); ap.add_argument('--price-frontier-targets',type=int,default=16); ap.add_argument('--price-frontier-packages-per-target',type=int,default=18); ap.add_argument('--portfolio-depth',type=int,default=15); ap.add_argument('--portfolio-max-moves',type=int,default=3); ap.add_argument('--portfolio-beam-width',type=int,default=10); ap.add_argument('--portfolio-sims',type=int,default=1000); ap.add_argument('--portfolio-confirm-sims',type=int,default=5000); ap.add_argument('--portfolio-confirm-top',type=int,default=3); ap.add_argument('--trade-review-depth',type=int,default=3); ap.add_argument('--trade-review-quick-sims',type=int,default=200); ap.add_argument('--trade-review-confirm-sims',type=int,default=50000); ap.add_argument('--trade-review-search-depth',type=int,default=60); ap.add_argument('--robustness-seeds',type=int,default=0); ap.add_argument('--robustness-sims',type=int,default=500); ap.add_argument('--strategic-posture',default='AUTO'); ap.add_argument('--seed',type=int,default=20260821); a=ap.parse_args(); out=Path(a.output); raw=Path(a.team_improvement_input) if a.team_improvement_input else out.with_suffix('.team-improvement.json')
