...
def build_board(source,a,reviews):
    ...
    stability_seeds=[
        int(a.seed)+i*1009
        for i in range(max(0,int(a.actionability_stability_seeds)))
    ]
    stability=focal_utility_stability.evaluate(
        bilateral_rows,
        a.focus_user_id,
        simulations=int(a.actionability_stability_sims),
