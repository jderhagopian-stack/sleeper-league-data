...
def build_board(source,a,reviews):
    ...
    b['model_version']=MODEL_VERSION; b['negotiation_frontier']=frontier; b['best_price_overlap']=frontier.get('best_price_overlap')
    ...
    b.setdefault('capability_status',{}).update({'negotiation_frontier':True,'discrete_price_frontier':True,'progressive_price_frontier_search':True,'actionable_vs_explore_vs_theoretical_trade_separation':True,'high_impact_price_gap_bucket':True})
    b.setdefault('provenance',{})['negotiation_frontier_bilateral_authority']='canonical governed counterparty shared utility'
    return b
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--focus-user-id',required=True); ap.add_argument('--output',required=True); ap.add_argument('--team-improvement-input'); ap.add_argument('--quick-sims',type=int,default=500); ap.add_argument('--confirm-sims',type=int,default=2500); ap.add_argument('--trade-screen',type=int,default=75); ap.add_argument('--waiver-screen',type=int,default=50); ap.add_argument('--confirm-top',type=int,default=15); ap.add_argument('--trade-packages-per-target',type=int,default=12); ap.add_argument('--price-frontier-targets',type=int,default=16); ap.add_argument('--price-frontier-packages-per-target',type=int,default=18); ap.add_argument('--portfolio-depth',type=int,default=15); ap.add_argument('--portfolio-max-moves',type=int,default=3); ap.add_argument('--portfolio-beam-width',type=int,default=10); ap.add_argument('--portfolio-sims',type=int,default=1000); ap.add_argument('--portfolio-confirm-sims',type=int,default=5000); ap.add_argument('--portfolio-confirm-top',type=int,default=3); ap.add_argument('--trade-review-depth',type=int,default=3); ap.add_argument('--trade-review-quick-sims',type=int,default=200); ap.add_argument('--trade-review-confirm-sims',type=int,default=50000); ap.add_argument('--trade-review-search-depth',type=int,default=60); ap.add_argument('--robustness-seeds',type=int,default=0); ap.add_argument('--robustness-sims',type=int,default=500); ap.add_argument('--seed',type=int,default=20260821); a=ap.parse_args(); out=Path(a.output); raw=Path(a.team_improvement_input) if a.team_improvement_input else out.with_suffix('.team-improvement.json')
