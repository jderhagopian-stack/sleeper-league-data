#!/usr/bin/env python3
"""Build a reconstructed-at-time GM 3.0 input bundle for a historical FSFFL trade.
...
DATA = ROOT / "data"
SCRIPT = ROOT / "script"
MODEL_VERSION = "FSFFL-Historical-GM3-Bundle-Builder-1.1"
...
def build_player_values(rosters, players, prior, baselines, source, season):
    pff = {
    ...
    for pid in roster_pids:
        ...
        values[pid] = {
            ...
            "name": name,
            "position": pos,
            "age": round(age_at(meta, int(season)), 1),
            "years_exp": max(0.0, float(meta.get("years_exp") or 0) - max(0, datetime.now(timezone.utc).year - int(season))),
...
def build(season: str, transaction_id: str, source_path: Path | None = None):
    ...
    schedule = neutral_schedule(int(season))
    ...
    return {
        ...
        "provenance": {
            ...
            "reconstruction_uses_only_pre_transaction_or_prior_completed_season_information": True,
            "same_season_results_used": False,
