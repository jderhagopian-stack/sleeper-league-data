...
MODEL_VERSION='FSFFL-Season-Simulator-Report-1.0'
...
def render(input_path,uid,name,output):
    ...
    cards=[
        ...
        kpi_card(s,'Expected PF',f'{pf:,.0f}','blue'),
        kpi_card(s,'Playoff Odds',f'{po*100:.1f}%','positive' if po>=.65 else 'warning'),
        kpi_card(s,'Bye Odds',f'{bye*100:.1f}%','positive' if bye>=.25 else 'neutral'),
        kpi_card(s,'Title Odds',f'{title*100:.1f}%','positive' if title>=.12 else 'neutral'),
        kpi_card(s,'Division Odds',f'{div*100:.1f}%','blue'),
    ]
    ...
    right=[P(s,'TITLE LANDSCAPE','FS_Section'),lt,Spacer(1,5),P(s,'MODEL QUALITY','FS_Section'),P(s,f"Validation passed: {validation.get('validation_passed')}. {coverage}",'FS_Small'),Spacer(1,3),P(s,'READING GUIDE','FS_Section'),P(s,'All probabilities and expected records are direct Simulator outputs. The report ranks and formats them but does not alter the simulation or create new football judgments.','FS_Small')]
    ...
    doc.build(story,onFirstPage=lambda c,d: footer(c,f'{MODEL_VERSION} | Raw simulator output | Presentation-only'))
