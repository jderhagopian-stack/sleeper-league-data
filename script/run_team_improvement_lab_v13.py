...
def main():
    ppt=_pop_cli_int('--trade-packages-per-target',DEFAULT_TRADE_PACKAGES_PER_TARGET); out=output_path_from_argv(); base=load_base(); base.MODEL_VERSION=MODEL_VERSION; saved=base.evaluate_row
    ...
    def ev(row,uid,dl,lo,ra,mi,bl,b,s,seed):
        if row.get('channel')!='WAIVER' or not row.get('native_full_projection'):return saved(row,uid,dl,lo,ra,mi,bl,b,s,seed)
