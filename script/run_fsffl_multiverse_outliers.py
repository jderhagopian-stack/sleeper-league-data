...
def main():
    ...
    raw_schedule = core.load_json(DATA / "schedule.json") or core.load_json(DATA / "full_schedule.json") or core.load_json(DATA / "matchups.json")
    if not league or not rosters or not users or not players or raw_schedule is None:
        raise RuntimeError("Missing core FSFFL data")
    ...
    for week in all_weeks:
        ...
        for rid in roster_ids:
            ...
            for pid, arr in point_map.items():
                row = meta[pid]
                info = {
                    "player_id": str(pid), "player": row.get("name"), "position": row.get("position"),
                    "nfl_team": row.get("nfl_team"), "roster_id": int(rid),
                    "fsffl_team": roster_dir[rid]["team_name"], "baseline_ppg": round(float(row.get("mean", 0.0)), 3),
                }
                player_info[pid] = info
                if week in reg_weeks:
                    player_season.setdefault(pid, np.zeros(n_sims, dtype=np.float32))
                    player_season[pid] += arr
    ...
    reg_idx = [week_to_i[w] for w in reg_weeks]
    reg_scores = scores[:, reg_idx, :]
    pf = reg_scores.sum(axis=1, dtype=np.float64)
    ...
    for week in reg_weeks:
        ...
        for a, b in by_week.get(week, []):
            ai, bi = rid_to_i[a], rid_to_i[b]
            sa, sb = scores[:, wi, ai], scores[:, wi, bi]
            aw = sa >= sb
            ...
            if biggest_margin is None or m > biggest_margin["margin"]:
                ...
                biggest_margin = {
                    "simulation_id": sid(k), "week": int(week),
                    "winner": roster_dir[i_to_rid[winner_i]]["team_name"], "loser": roster_dir[i_to_rid[loser_i]]["team_name"],
                    "winner_score": round(float(scores[k, wi, winner_i]), 3), "loser_score": round(float(scores[k, wi, loser_i]), 3), "margin": round(m, 3),
                }

    orders = prod.fast_seed_orders(wins, pf, roster_ids)
    ...
    max_team_week = team_week_extreme(np.argmax)
    min_team_week = team_week_extreme(np.argmin)
    ...
    max_season_pf = season_pf_extreme(np.argmax)
    min_season_pf = season_pf_extreme(np.argmin)
    ...
    sim = np.arange(n_sims); top6 = orders[:, :6]
    w1, w2, w3 = [week_to_i[x] for x in playoff_weeks]
    ...
    semi1 = np.where(scores[sim,w2,s1] >= scores[sim,w2,low], s1, low)
    semi2 = np.where(scores[sim,w2,s2] >= scores[sim,w2,high], s2, high)
    champion = np.where(scores[sim,w3,semi1] >= scores[sim,w3,semi2], semi1, semi2)
    champion_seed = seed_num[sim, champion]

    title_counts = np.zeros(n_teams, dtype=np.int64); np.add.at(title_counts, champion, 1); title_rates = title_counts / n_sims
    rare_i = sorted(set(int(x) for x in champion), key=lambda i: title_rates[i])[0]
    s = int(np.flatnonzero(champion == rare_i)[0]); w = int(wins[s,rare_i])
    ...
    for pid, arr in player_season.items():
        info = player_info[pid]; k = int(np.argmax(arr)); val = float(arr[k])
        cand = {"simulation_id": sid(k), **info, "season_points": round(val, 3), "simulated_ppg": round(val / len(reg_weeks), 3)}
    ...
    output = {
        "generated_at_utc": core.now_utc(), "model_version": "FSFFL-Season-Simulator-1.0-multiverse",
        "season": season, "simulations": n_sims, "rng_seed": seed, "opponent_adjustment_source": adjustment_source,
        "runtime_seconds": round(time.perf_counter() - started, 3),
        "notes": {
            "simulation_id": "1-based ID within this deterministic multiverse pass.",
            "surprise_superstar_rule": "Highest regular-season total among players with baseline PPG <= 8.0.",
            "historical_records": "Simulation extremes only; compare with the FSFFL historical record book before labeling a real league record.",
        },
        "player_extremes": {"highest_single_week": max_player_week, "highest_regular_season_total": max_player_season, "unexpected_superstar_season": surprise_superstar},
        "team_extremes": {"highest_single_week_score": max_team_week, "lowest_single_week_score": min_team_week, "highest_regular_season_points": max_season_pf, "lowest_regular_season_points": min_season_pf, "best_regular_season_record": best_record, "worst_regular_season_record": worst_record, "biggest_margin_of_victory": biggest_margin, "best_team_to_miss_playoffs": best_miss},
        "playoff_extremes": {"rarest_champion": rarest_champion, "highest_seed_champion": highest_seed_champion},
    }
    out = SIM_ROOT / season / "outputs" / "multiverse_outliers.json"
    write_json(out, output)
    print(f"Multiverse tracker complete: {n_sims:,} universes in {output['runtime_seconds']:.3f}s")
    print(f"Wrote {out}")
