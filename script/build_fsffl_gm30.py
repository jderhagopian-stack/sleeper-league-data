...
def weighted(items):
    ...
    if not present or poss<=0: return 0.5,0.0
...
def age_score(pos, age):
    ...
    if lo<=age<=hi: return .78
    if age<lo: return clamp(.52+.20*(age/max(lo,1)))
...
def resolve_owner_rows(payload):
    return payload if isinstance(payload,list) else (payload.get("owners") or payload.get("teams") or []) if isinstance(payload,dict) else []

def sim_bridge(cfg, standings, lineups, projections):
    teams=standings.get("teams",[]) if isinstance(standings,dict) else []
    by_uid={str(t.get("user_id")):t for t in teams}
    by_manager={norm(t.get("manager")):t for t in teams}
    ...
    for t in teams:
        ...
        rows.append({
            **t,
            ...
            "competitive_window":"elite_contender" if sf(t.get("championship_probability"))>=.18 else
...
def lineup_usage(lineups):
    starts=defaultdict(int); slots=defaultdict(Counter); means=defaultdict(list)
...
def owner_profiles_v3(cfg, owners, sim, trade_summary):
    ...
    trade_by_uid={}
    ts=resolve_owner_rows(trade_summary)
    for x in ts:
        trade_by_uid[str(x.get("user_id"))]=x
    rows=[]
    for o in resolve_owner_rows(owners):
        ...
        negotiation=weighted([(activity,.36),(clamp(recent/20),.24),(multi,.20),(initiation,.20)])[0]
        rows.append({
            "user_id":uid,"manager":o.get("manager"),"team_name":o.get("team_name"),
...
def opportunity_radar(cfg, assets, projections, owner_v3, sim, football_intel):
    ...
    if isinstance(football_intel,list):
        ...
        rows=football_intel.get("signals") or football_intel.get("players") or []
        if isinstance(rows,list):
            for x in rows: intel[str(x.get("player_id") or x.get("sleeper_id") or "")]=x
    ...
    for p in players:
        ...
        if pr:
            ...
            # moderate volatility is upside-friendly for cheap assets, extreme volatility is penalized
            asym=clamp(.70-.35*abs(cv-.65))
        fi=p.get("football_intelligence") or {}
        usage=fi.get("usage_and_snaps") or {}
        vals=[]
        for v in [usage.get("usage_signal"),usage.get("snap_signal"),
                  (fi.get("manual_news_signal") or {}).get("signal"),
                  (intel.get(pid) or {}).get("signal")]:
            if v is not None:
                v=sf(v); vals.append(clamp((v+1)/2 if v<0 else v))
        ...
        comp,coverage=weighted([
            (disagreement,.22),(role_news,.28),(proj_strength,.18),(asym,.08),
            (momentum,.08),(age,.10),(access,.06)
        ])
        ...
        bust=weighted([(1-disagreement,.22),(None if role_news is None else 1-role_news,.26),
                       (None if proj_strength is None else 1-proj_strength,.20),(1-age if age is not None else None,.16),
                       (1-momentum,.16)])[0]
        cats=[]
        th=cfg["alert_thresholds"]
        for typ,sc,t in [("HIDDEN_GEM",hidden,th["hidden_gem"]),("BREAKOUT_WATCH",breakout,th["breakout"]),
                         ("BUY_LOW",buylow,th["buy_low"]),("BUST_RISK",bust,th["bust_risk"])]:
            if sc*100>=t: cats.append({"type":typ,"score":round(sc*100,1)})
        # Young players with a strong underlying profile but modest current projection:
        # stash/watch rather than force an immediate breakout recommendation.
        young = (p.get("position")=="QB" and sf(p.get("age"),99)<=25) or                 (p.get("position")=="RB" and sf(p.get("age"),99)<=23) or                 (p.get("position")=="WR" and sf(p.get("age"),99)<=24) or                 (p.get("position")=="TE" and sf(p.get("age"),99)<=25)
        early_not_yet=weighted([(disagreement,.30),(role_news,.24),(age,.24),
                                (None if proj_strength is None else 1-proj_strength,.22)])[0]
        ...
        rows.append({
            "player_id":pid,"name":p.get("name"),"position":p.get("position"),"nfl_team":p.get("nfl_team"),
            "age":p.get("age"),"owner_manager":p.get("current_owner_manager"),"owner_team":p.get("current_owner_team"),
            "market_dynasty":market,"fsffl_value":fsffl,"trend_30_day":trend,
            ...
            "gm30_vs_market_pct":round(100*((fsffl * (1 + max(-0.12,min(0.12,(comp-.5)*.24)))/market)-1),1) if market else None,
            "signal_score":round(comp*100,1),"confidence":round(confidence,3),"coverage":round(coverage,3),
            ...
            "components":{"market_disagreement":round(disagreement,3),"role_usage_news":None if role_news is None else round(role_news,3),
                          "projection_strength":None if proj_strength is None else round(proj_strength,3),
                          "upside_asymmetry":None if asym is None else round(asym,3),"market_momentum":round(momentum,3),
                          "age_curve":None if age is None else round(age,3),"owner_accessibility":round(access,3)}
        })
    return sorted(rows,key=lambda x:max([c["score"] for c in x["categories"]] or [x["signal_score"]]),reverse=True)
...
def roster_arbitrage(cfg, assets, projections, rosters, players):
    ...
    for r in rosters if isinstance(rosters,list) else []:
        rid=str(r.get("roster_id")); pids=[str(x) for x in (r.get("players") or [])]
        ...
        rows.append({
            "roster_id":rid,
            "bottom_assets":[{
                "player_id":pid,"name":a.get("name") or (player_meta.get(pid) or {}).get("full_name"),
                "position":a.get("position") or (player_meta.get(pid) or {}).get("position"),
                "fsffl_value":a.get("fsffl_value"),"projection_ppg":round(pr.get("projection_ppg",0),2) if pr else None,
                "roster_slot_utility":round(u,4)
            } for u,pid,a,pr in ranked[:3]]
        })
    # identify unrostered projected players as waiver candidates
    waiver=[]
    for pid,pr in pf.items():
        if pid in rostered: continue
        meta=player_meta.get(pid,{}) if isinstance(player_meta,dict) else {}
        waiver.append({
            "player_id":pid,"name":pr.get("name") or meta.get("full_name"),"position":pr.get("position") or meta.get("position"),
            "projection_ppg":round(pr.get("projection_ppg",0),2),"projection_cv":round(pr.get("projection_cv",0),3),
...
def trade_routes(cfg, radar, owner_v3, sim, user_uid):
    ...
    for p in radar:
        uid=None
        # radar doesn't expose owner uid, recover by owner manager
        for o in owner_v3:
            if norm(o.get("manager"))==norm(p.get("owner_manager")):
                uid=str(o.get("user_id")); break
        if not uid or uid==str(user_uid): continue
        own=owner_by_uid.get(uid,{})
        if not p.get("categories"): continue
        ...
        candidates.append({
            "target_player_id":p["player_id"],"target":p["name"],"position":p["position"],
            "owner_manager":p.get("owner_manager"),"owner_team":p.get("owner_team"),
            "primary_signal":best,"target_market_value":p.get("market_dynasty"),"target_fsffl_value":p.get("fsffl_value"),
            "counterparty_acceptance_proxy":round(acceptance,3),"route_priority":round(100*route,1),
            "suggested_offer_shape":"future_pick_heavy" if sf(own.get("pick_appetite_score"),50)>=58 else
...
def championship_marginal_value(cfg, radar, sim, user_uid):
    ...
    for p in radar:
        ...
        # Approximate replacement improvement around a 7 ppg flexible-starter benchmark.
        delta_ppg=max(0,ppg-7.0)
        season_pts=delta_ppg*14
        champ_delta=min(cfg["trade"]["max_probability_delta_per_trade"],
...
def decision_center(cfg, radar, routes, rosterarb, sim, user_uid):
    ...
    for p in radar:
        ...
        if best["type"]=="BUST_RISK":
            if score>=cfg["alert_thresholds"]["act_now"]: fades.append(item)
            else: watches.append(item)
        elif score>=cfg["alert_thresholds"]["act_now"]:
            acts.append(item)
        elif score>=cfg["alert_thresholds"]["watch"]:
            watches.append(item)
    no_action = None
    if not acts and not fades:
        no_action = "NO MATERIAL GM 3.0 ACTION SIGNALS"
    return {
...
def decision_journal_snapshot(decisions):
    """
    Merge current high-priority recommendations into a persistent journal.
    Existing review/outcome fields are preserved across daily rebuilds.
    """
    now=datetime.now(timezone.utc).isoformat()
    existing=load(OUT/"decision_journal.json",{}) or {}
    old_by_id={str(x.get("decision_id")):x for x in existing.get("entries",[]) if x.get("decision_id")}
    current_ids=set()
        ...
        for d in decisions.get(bucket,[])[:20]:
            ...
            if did in old_by_id:
                ...
                old_by_id[did]={
                    ...
                    "status":"OPEN","decision_type":bucket.upper(),
                    "player_id":d.get("player_id"),"player_name":d.get("name"),
                    "initial_signal":d.get("signal"),"latest_signal":d.get("signal"),
    ...
    return {"generated_at_utc":now,"entries":entries,
            "note":"Persistent self-audit journal. Outcome/process review fields survive daily rebuilds."}

def validation(cfg, inputs, outputs):
    checks=[]
    required=["asset_values","owner_behavior","rosters","players","sim_standings","sim_lineups","sim_projections"]
    for key in required:
...
def main():
    ...
    sim=sim_bridge(cfg,inputs["sim_standings"],inputs["sim_lineups"],inputs["sim_projections"])
    owners=owner_profiles_v3(cfg,inputs["owner_behavior"],sim,inputs.get("trade_summary"))
