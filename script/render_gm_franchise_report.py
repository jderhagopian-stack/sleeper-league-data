...
MODEL_VERSION='FSFFL-GM-Franchise-Report-1.0'
...
def render(index_path, uid, slug, output):
    ...
    state=str(cc.get('team_state') or 'unknown').replace('_',' ').title()
    state_bg=LIGHT_GREEN if 'Contender' in state else LIGHT_BLUE
    banner=Table([[P(s,'FRANCHISE STATE','FS_WhiteLabel'),P(s,f'<b>{state}</b> &nbsp;&nbsp; Contender score {safe_float(cc.get("contender_score")):.3f} | Dynasty roster score {safe_float(cc.get("dynasty_roster_score")):.3f}','FS_Body')]],colWidths=[1.5*inch,5.94*inch])
    ...
    cards=[
        kpi_card(s,'Starter Redraft',f"{safe_float(cc.get('starter_redraft_value')):,.0f}",'blue'),
        kpi_card(s,'Starter Dynasty',f"{safe_float(cc.get('starter_dynasty_value')):,.0f}",'blue'),
        kpi_card(s,'Contender Score',f"{safe_float(cc.get('contender_score')):.3f}",'positive' if safe_float(cc.get('contender_score'))>=.6 else 'neutral'),
        kpi_card(s,'Dynasty Score',f"{safe_float(cc.get('dynasty_roster_score')):.3f}",'positive' if safe_float(cc.get('dynasty_roster_score'))>=.6 else 'neutral'),
    ]
    ...
    need_rows=[[P(s,'POSITION','FS_CardLabel'),P(s,'NEED SCORE','FS_CardLabel')]]+[[P(s,x.get('position'),'FS_Body'),P(s,f"{safe_float(x.get('need_score')):.3f}",'FS_Body')] for x in needs[:4]]
    ...
    left=[P(s,'ROSTER NEEDS','FS_Section'),need_tbl,Spacer(1,5),P(s,'OBJECTIVE WEIGHTS','FS_Section'),P(s,wtxt,'FS_Body')]
    assets=cc.get('highest_break_glass_assets') or []
    asset_rows=[[P(s,'ASSET','FS_CardLabel'),P(s,'STATUS','FS_CardLabel'),P(s,'BREAK GLASS','FS_CardLabel'),P(s,'LIQ.','FS_CardLabel')]]
    for a in assets[:6]:
    ...
    right=[P(s,'HIGHEST STRATEGIC / BREAK-GLASS ASSETS','FS_Section'),asset_tbl,Spacer(1,4),P(s,'READING GUIDE','FS_Section'),P(s,'Break-glass value is the GM threshold for moving a strategically important asset. Liquidity reflects how readily value can be redeployed. These are raw GM outputs; the PDF does not independently rescore the roster.','FS_Small')]
    ...
    doc.build(story,onFirstPage=lambda c,d: footer(c,f'{MODEL_VERSION} | Raw GM 3.0 output | Presentation-only'))
