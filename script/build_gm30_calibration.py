...
Writes only to data/gm/. Legacy GM 2.2-derived datasets may be read as
temporary compatibility inputs, but GM 3.0 is the primary GM engine.
"""
...
def summarize(rows, keys):
    ...
    for key, g in groups.items():
        ...
        row.update({
            "sample": g["n"],
            "rostered_rate": round(g["rostered"] / n, 3),
...
def main():
    ...
    report = {
        ...
        "architecture": {
            "engine_role": "primary_gm_engine",
            "output_root": "data/gm",
            "legacy_gm22_policy": "compatibility_input_only_until_replaced",
        },
        "draft_proxy": {
            "sample": len(draft or []),
        ...
        "true_prospect_feature_backtest": {
            ...
            "why_it_matters": "Prospect feature weights should not be optimized against retention proxies.",
        },
        "calibration_policy": {
            "allowed_now": [
                "owner behavior priors", "pick-location priors", "waiver aggression priors",
                "retention/roster survival sanity checks"
            ],
            "not_claimed_without_true_history": [
                "causal prospect hit rates", "Puka-style sleeper precision", "college metric coefficients"
            ],
            "future_metrics": [
                "precision_at_k", "recall_of_breakouts", "false_positive_rate",
                "market_value_gain_30_90_365d", "Brier/log-loss where probabilistic labels exist"
            ],
        },
    }
    with (OUT / "calibration_report.json").open("w", encoding="utf-8") as f:
        json.dump(report, f, indent=2)
    print(
        f"GM 3.0 calibration audit complete: {len(draft or [])} draft proxy rows; "
        f"true prospect backtest={report['true_prospect_feature_backtest']['status']}"
    )
