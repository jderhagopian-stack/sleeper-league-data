...
MODEL_VERSION='FSFFL-Multiverse-Outliers-Report-1.0'
...
def render(input_path,output):
    ...
    story=[P(s,'FSFFL MULTIVERSE OUTLIERS','FS_Title'),P(s,f"Season {d.get('season')} | {int(safe_float(d.get('simulations'))):,} simulated universes | {d.get('model_version')}",'FS_Sub'),Spacer(1,5)]
    ...
    right=[P(s,'TEAM EXTREMES','FS_Section'),tt,Spacer(1,5),P(s,'READING GUIDE','FS_Section'),P(s,'These are extreme outcomes observed across the deterministic simulation multiverse, not forecasts of the most likely outcome. Historical league records must be checked separately before calling any simulated extreme an FSFFL record.','FS_Small')]
    ...
    doc.build(story,onFirstPage=lambda c,x: footer(c,f'{MODEL_VERSION} | Raw multiverse outlier output | Presentation-only'))
def main():
