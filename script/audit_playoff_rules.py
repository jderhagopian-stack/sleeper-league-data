...
def main():
    ...
    assert sim.playoff_round_count(4) == 2
    assert sim.playoff_round_count(6) == 3
    assert sim.playoff_round_count(8) == 3
    ...
    assert sim.configured_playoff_weeks({"settings":{"playoff_teams":8,"playoff_week_start":14}}) == [14,15,16]
    assert sim.eligible("K", "K")
    assert sim.eligible("DST", "DEF")
    assert sim.eligible("WR", "FLEX")
    assert sim.eligible("QB", "SUPER_FLEX")
    assert not sim.eligible("QB", "FLEX")
    ...
    payload = {
        ...
        "configured_playoff_weeks_derive_from_rules": True,
        "canonical_flex_k_dst_eligibility": True,
