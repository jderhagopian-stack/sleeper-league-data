...
MODEL_VERSION='FSFFL-Market-Movement-Report-1.0'
...
def render(input_path,output):
    ...
    story=[P(s,'FSFFL MARKET MOVEMENT & INTELLIGENCE REPORT','FS_Title'),P(s,f"Raw market trends and football-intelligence adjustments | {d.get('model_version')}",'FS_Sub'),Spacer(1,5)]
    ...
    right=[]; right+=block('STRONGEST POSITIVE INTELLIGENCE ADJUSTMENTS',intel_up,'intel'); right += [Spacer(1,5)]; right+=block('STRONGEST NEGATIVE INTELLIGENCE ADJUSTMENTS',intel_down,'intel'); right += [Spacer(1,5),P(s,'READING GUIDE','FS_Section'),P(s,'These are direct rankings of existing raw fields. The report does not invent a composite score or call a player a buy/sell/breakout. Conversational analysis should interpret the underlying trend, usage, injury and news context.','FS_Small')]
    ...
    doc.build(story,onFirstPage=lambda c,d: footer(c,f'{MODEL_VERSION} | Raw market + football intelligence fields | Presentation-only'))
def main():
