...
MODEL_VERSION='FSFFL-League-Power-Report-1.1'
...
def render(input_path,output):
    ...
    rows=[[P(s,'#','FS_CardLabel'),P(s,'TEAM','FS_CardLabel'),P(s,'WINS','FS_CardLabel'),P(s,'PF','FS_CardLabel'),P(s,'PLAYOFF','FS_CardLabel'),P(s,'BYE','FS_CardLabel'),P(s,'DIVISION','FS_CardLabel'),P(s,'TITLE','FS_CardLabel')]]
    ...
    tbl.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),NAVY),('TEXTCOLOR',(0,0),(-1,0),WHITE),('GRID',(0,0),(-1,-1),.35,MID_GRAY),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('LEFTPADDING',(0,0),(-1,-1),3),('RIGHTPADDING',(0,0),(-1,-1),3),('TOPPADDING',(0,0),(-1,-1),4),('BOTTOMPADDING',(0,0),(-1,-1),4)])); story += [P(s,'SIMULATOR RANKING TABLE','FS_Section'),tbl,Spacer(1,5)]
    val=d.get('validation') or {}; story += [P(s,'MODEL QUALITY','FS_Section'),P(s,f"Validation passed: {val.get('validation_passed')}. This report only sorts and formats direct simulator outputs; it does not assign independent contender tiers, grades, or a separate power score.",'FS_Small')]
