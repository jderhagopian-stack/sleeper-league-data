#!/usr/bin/env python3
"""FSFFL Counter & Market Sweep 1.5 — bilateral rationality gate.

Builds on 1.4. Candidate trades are no longer ranked only by focal-team utility.
Every simulated candidate is also evaluated from the buyer's current GM 3.0
competitive state. Deals that materially conflict with the buyer's current
objective are excluded from the actionable Top 5 and surfaced separately as
STATE_CHANGE_DEPENDENT opportunities.

This is a strategic plausibility heuristic, not a calibrated acceptance
...
MODEL_VERSION = "FSFFL-Counter-Market-Sweep-1.5"
...
def buyer_rationality(row: Dict[str, Any], dl) -> Dict[str, Any]:
    ...
    # Current-state utility: title preservation dominates for contenders;
    # future value dominates for rebuilders; retools balance both.
    ...
    # A second lens asks whether a rejected deal becomes rational under a
    # plausible strategic pivot toward retool/rebuild.
    ...
    # Keep this explicitly heuristic rather than pretending to know a human's
...
def main():
    ...
    # Simulate a broader set than the report Top 5 so buyer-side filtering can
    # replace unrealistic focal-team-optimal deals rather than just deleting them.
    n = max(8, args.shortlist, args.finalists)
    with tempfile.TemporaryDirectory() as td:
        ...
        subprocess.run([
            ...
            "--shortlist", str(n), "--finalists", str(n), "--seed", str(args.seed),
            "--output", str(raw_out),
        ], check=True, stdout=subprocess.DEVNULL)
        report = json.loads(raw_out.read_text(encoding="utf-8"))

    v13 = load_module(V13_PATH, "market_sweep_v13_for_v15")
    engine = v13.load_module(v13.BASE_ENGINE, "market_sweep_base_for_v15")
    ...
    actionable = [r for r in rows if r["buyer_rationality"]["current_state_viable"]]
    actionable.sort(key=lambda r: (r["post_sim_score"], r["buyer_rationality"]["heuristic_acceptance_fit_score"]), reverse=True)
    pivot = [r for r in rows if not r["buyer_rationality"]["current_state_viable"] and r["buyer_rationality"]["state_change_viable"]]
    pivot.sort(key=lambda r: r["post_sim_score"], reverse=True)
    ...
    if not top5:
        action = "DECLINE"
    elif current and current.get("championship_equity_constraint") == "PASS" and current["buyer_rationality"]["current_state_viable"]:
        best = top5[0]
    ...
    report.setdefault("candidate_counts", {})["buyer_current_state_viable"] = len(actionable)
