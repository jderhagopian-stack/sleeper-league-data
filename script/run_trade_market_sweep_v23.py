...
focal franchise's continuous state-aware objective. Historical owner behavior
is useful evidence, but its acceptance adjustment is conditioned on the buyer's
current competitive state and cannot override current-state utility.

Descriptive state labels do not create hard focal-utility cliffs. The canonical
post-simulation score already applies continuous objective weights; only the
separate hard championship-equity constraint remains categorical for contenders.
"""
...
V22_PATH=SCRIPT/'run_trade_market_sweep_v22.py';V21_PATH=SCRIPT/'run_trade_market_sweep_v21.py';V20_PATH=SCRIPT/'run_trade_market_sweep_v20.py';V19_PATH=SCRIPT/'run_trade_market_sweep_v19.py';V18_PATH=SCRIPT/'run_trade_market_sweep_v18.py';V16_PATH=SCRIPT/'run_trade_market_sweep_v16.py'
MODEL_VERSION='FSFFL-Counter-Market-Sweep-1.17';NEGOTIATION_RANKING=SCRIPT/'negotiation_ranking.py'
...
def band(score):return 'HIGH' if score>=.68 else 'MEDIUM' if score>=.48 else 'LOW' if score>=.28 else 'VERY_LOW'
...
def focal_state_beneficial(row):
    """Use the continuous focal objective; state labels are descriptive only."""
    state=focal_current_state(row);post=sf(row.get('post_sim_score'))
    if post<=0:return False
    if state in {'contender','elite_contender'} and row.get('championship_equity_constraint')=='FAIL':return False
    return True

def state_condition_behavior(row,br):
    sig=dict(br.get('owner_behavior') or {});static_adj=sf(sig.get('adjustment'));base=sf(br.get('state_utility_acceptance_fit_score'),sf(br.get('heuristic_acceptance_fit_score'),.5)-static_adj);state=str(br.get('buyer_state') or 'unknown')
    buyer_receives=[str(x) for x in (row.get('outgoing_assets') or [])];buyer_sends=[str(x) for x in (row.get('return_assets') or [])];recv_picks=sum(x.startswith('pick:') for x in buyer_receives);send_picks=sum(x.startswith('pick:') for x in buyer_sends);net_pick_in=recv_picks-send_picks
    if state=='elite_contender':compat=1.0 if net_pick_in<0 else .55 if net_pick_in>0 else .75
    elif state=='contender':compat=.90 if net_pick_in<0 else .60 if net_pick_in>0 else .75
    elif state=='retool':compat=1.0 if net_pick_in>0 else .40 if net_pick_in<0 else .70
    elif state=='rebuild':compat=1.0 if net_pick_in>0 else .15 if net_pick_in<0 else .60
    else:compat=.50
    conditioned=static_adj*compat
    if state in {'rebuild','retool'} and net_pick_in<0 and conditioned>0:conditioned=min(conditioned,.01)
    if state in {'contender','elite_contender'} and net_pick_in>0 and conditioned>0:conditioned=min(conditioned,.02)
    score=round(clamp(base+conditioned,0.0,1.0),4)
    sig.update({'static_historical_adjustment':round(static_adj,4),'state_conditioned_adjustment':round(conditioned,4),'adjustment':round(conditioned,4),'current_state':state,'state_compatibility_weight':round(compat,3),'buyer_net_pick_in':int(net_pick_in),'state_conditioning_note':"Aggregate historical behavior is attenuated when it conflicts with the manager's current competitive state."})
...
def patch_v18(mod):
    ...
    def adjusted(base_mod,row,dl,beh,meta):return state_condition_behavior(row,original(base_mod,row,dl,beh,meta))
    mod.adjusted_buyer_rationality=adjusted;return mod

def patch_v16(mod):
    original=mod.focal_viable
    def focal_viable(row):
        ok=original(row);beneficial=focal_state_beneficial(row);row['focal_current_state_beneficial']=bool(beneficial);row['focal_current_state']=focal_current_state(row);return bool(ok and beneficial)
    mod.focal_viable=focal_viable;return mod
...
def patch_v21_selectors(mod):
    ...
    def prepare(rows):
        for r in rows:
            ...
            if br:
                state_condition_behavior(r,br);r['acceptance_likelihood']=br.get('heuristic_acceptance_fit');r['negotiation_ranking']=recompute_negotiation_ranking(r)
    ...
    def normal(viable,swing):
        prepared=[r for r in prepare(list(viable)) if focal_state_beneficial(r)];selected=[];counts=Counter();used_families=set();swing_family=mod.negotiation_family_key(swing) if swing else None
        for row in prepared:
            fam=mod.negotiation_family_key(row)
            if swing_family and fam==swing_family:continue
            if fam in used_families:continue
            uid=str(row.get('buyer_user_id') or '')
            if counts[uid]>=mod.MAX_NORMAL_OPTIONS_PER_BUYER:continue
            selected.append(row);used_families.add(fam);counts[uid]+=1
            if len(selected)==4:break
        return selected
    def swing(viable):return original_swing(prepare(list(viable)))
...
def main():
    v22=load_module(V22_PATH,'market_sweep_v22_for_v117');original_v22_loader=v22.load_module
    def patched_v22_loader(path:Path,name:str):
        mod=original_v22_loader(path,name)
        if Path(path)==V21_PATH:
            mod=patch_v21_selectors(mod);original_v21_loader=mod.load_module
                    ...
                    def patched_v20_loader(p3:Path,n3:str):
                        ...
                        if Path(p3)==V19_PATH:
                            ...
                            def patched_v19_loader(p4:Path,n4:str):
                                ...
                                if Path(p4)==V18_PATH:m4=patch_v18(m4)
                                elif Path(p4)==V16_PATH:m4=patch_v16(m4)
                                return m4
