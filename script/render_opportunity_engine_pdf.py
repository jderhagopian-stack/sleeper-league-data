...
def render(board_path, output):
    ...
    best=b.get('best_move_available') or {}; besttrade=b.get('best_trade_opportunity') or {}; port=(b.get('portfolio_optimization') or {}).get('best_portfolio') or {}; td=td_decision(b)
    ...
    banner=Table([[P(s,'TOP ACTION','FS_WhiteLabel'),P(s,f"<b>{desc(best)}</b><br/>GM3 franchise-improvement score: <b>{score(best):+,.1f}</b> | Trade Decision: <b>{td if best.get('channel')=='TRADE' else 'N/A'}</b>",'FS_Body')]],colWidths=[1.15*inch,6.29*inch])
    ...
    cards=[kpi_card(s,'GM3 Improvement',f'{score(best):+,.0f}','positive' if score(best)>0 else 'neutral',1.78*inch),kpi_card(s,'Expected Wins',f'{sim_delta(best,"expected_wins"):+.2f}','positive' if sim_delta(best,"expected_wins")>0 else 'neutral',1.78*inch),kpi_card(s,'Championship Odds',f'{sim_delta(best,"championship_probability")*100:+.1f}%','positive' if sim_delta(best,"championship_probability")>0 else 'neutral',1.78*inch),kpi_card(s,'Playoff Odds',f'{sim_delta(best,"playoff_probability")*100:+.1f}%','positive' if sim_delta(best,"playoff_probability")>0 else 'neutral',1.78*inch)]
    ...
    brief=(f"The best currently executable move is {desc(best)}. Its displayed outcome deltas are bound to that exact action." if best.get('channel')!='HOLD' else "No evaluated trade currently clears both the seller's governed utility and this franchise's positive GM3 utility. Holding is therefore the current executable baseline, not a claim that no attractive players exist.")
    story += [P(s,'GM MORNING BRIEFING','FS_Section'),P(s,brief,'FS_Body'),Spacer(1,6)]
    if port:
