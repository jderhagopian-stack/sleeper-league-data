#!/usr/bin/env python3
"""
FSFFL GM Engine v1.0
==========================================

Purpose
-------
Builds a live, team-specific valuation matrix for every rostered player and
future draft pick in the FSFFL Dynasty league.

Market anchor:
  FantasyCalc public current-values endpoint
  - Dynasty + Redraft
  - 12 teams
  - 2 QBs / Superflex
  - 0.5 PPR
...
League/context adjustments:
  ...
  - Current starter/depth strength by position
  - Contender/rebuild status derived from current redraft starter strength
  - Owner historical trading/drafting/waiver preferences
  - Pick accumulation/spending tendencies
  - Current-owner replacement cost / starter dependency
  - Shallow-roster consolidation premium

Outputs
-------
data/market_values_fantasycalc.json
data/team_contender_profiles.json
data/fsffl_asset_values.json
data/owner_perceived_values.json
data/hsg_trade_opportunities.json
data/valuation_model_config.json
