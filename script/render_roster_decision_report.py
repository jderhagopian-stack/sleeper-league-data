...
def render(input_path,output):
    ...
    cards=[
        ...
        kpi_card(s,'Division Odds',arr(before.get('division_probability'),after.get('division_probability'),True),'positive' if safe_float(delta.get('division_probability'))>=0 else 'negative'),
    ...
    for label,key in [('Dynasty Market','market_dynasty_delta'),('Redraft Market','market_redraft_delta'),('Base Franchise','base_franchise_delta'),('Break-Glass','break_glass_delta')]: strategic_rows.append([P(s,label,'FS_Body'),P(s,f"{safe_float(strat.get(key)):+,.0f}",'FS_Body')])
    ...
    left=[P(s,'STRATEGIC VALUE DELTAS','FS_Section'),st,Spacer(1,5),P(s,'COMPETITIVE EXTERNALITY','FS_Section'),P(s,f"Focus title delta {pct(ext.get('focus_championship_probability_delta'))}; opponents gain {pct(ext.get('opponent_positive_championship_probability_delta_sum'))}; net swing against focus {pct(ext.get('net_title_equity_swing_against_focus'))}.",'FS_Body')]
