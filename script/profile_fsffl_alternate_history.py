...
Runs deterministic particle regressions across the real generated 2022 fork and
the established 2023 Puka fork, recording wall-clock time, isolated peak RSS,
state counts, compression ratios, and scaling efficiency. Each benchmark case
runs in a fresh subprocess so peak-memory measurements and long-lived Python
state from an earlier scale cannot contaminate later measurements.

This is an audit surface only: it calls the production generic orchestrator and
the same invariant validator used by CI.
"""
...
def _phase_metrics(report: Dict[str, Any], particles: int) -> List[Dict[str, Any]]:
    ...
    for phase in report.get("phase_audit") or []:
        ...
        growth = (
            round(unique / previous_unique, 6)
            if previous_unique not in (None, 0)
            else None
        )
        rows.append({
...
def _run_one(
    ...
    *,
    ...
    peak_phase = max(
        phase_metrics,
        key=lambda row: int(row.get("unique_states") or 0),
        default=None,
    )
    return {
...
def _run_isolated(label: str, particles: int, seed: int) -> Dict[str, Any]:
...
def _scaling_summary(case_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in case_rows:
        ...
        runtime_multiple = (
            float(row["elapsed_seconds"]) / base_elapsed if base_elapsed else None
        )
        ...
        if previous is not None:
            adjacent_particle_multiple = (
                float(row["particles"]) / float(previous["particles"])
            )
            previous_elapsed = float(previous["elapsed_seconds"])
            adjacent_runtime_multiple = (
                float(row["elapsed_seconds"]) / previous_elapsed
                if previous_elapsed else None
            )
            if adjacent_runtime_multiple is not None and adjacent_particle_multiple:
                ...
                superlinear = (
                    adjacent_runtime_multiple
                    > adjacent_particle_multiple * SUPERLINEAR_TOLERANCE
                )
        result.append({
            ...
            "adjacent_particle_multiple": (
                round(adjacent_particle_multiple, 3)
                if adjacent_particle_multiple is not None else None
            ),
            "adjacent_runtime_multiple": (
                round(adjacent_runtime_multiple, 3)
                if adjacent_runtime_multiple is not None else None
            ),
            "adjacent_scaling_efficiency": (
                round(scaling_efficiency, 4)
                if scaling_efficiency is not None else None
            ),
...
def profile(particle_scales: Iterable[int], *, seed: int = DEFAULT_SEED) -> Path:
    ...
    for label in cases:
        for particles in scales:
            result = _run_isolated(label, particles, seed)
            result["case"] = label
            rows.append(result)
    ...
    scaling = {
        label: _scaling_summary(case_rows)
    ...
    payload = {
        ...
        "purpose": (
            "Measure scaling, isolated peak memory, and state growth without "
            "changing model decisions or pruning probability mass."
        ),
        "configuration": {
    ...
    print(out)
    print(json.dumps(
        {"scaling_summary": scaling, "performance_alerts": alerts},
        indent=2,
        sort_keys=True,
    ))
    return out
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Profile generic Alternate History particle scaling"
    )
    parser.add_argument(
        "--particles",
        type=int,
    ...
    if args.single_case:
        if args.single_particles is None or args.single_particles <= 0 or args.result_path is None:
            raise ah.AlternateHistoryError(
                "single-case audit requires positive --single-particles and --result-path"
            )
